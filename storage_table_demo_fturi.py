...
resourcegroup_name = 'ftu'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'ftu'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'canadaeast'
...
car.PartitionKey = 'car'
car.RowKey = '002'
car.description = 'Mazda 3'
car.make='Mazda'
car.color='White'
car.year='2010'
car.cost = 25000
table_service.insert_entity('itemstable', car)
print('Created entry for Mazda 3...')

car = Entity()
car.PartitionKey = 'car'
car.RowKey = '003'
car.description = 'Malibu'
car.make='GM'
car.color='White'
car.year='2005'
car.cost = 200
table_service.insert_entity('itemstable', car)
print('Created entry for Malibu...\n')
...
coffee.RowKey = '005'
coffee.brand = 'Le Parisian'
coffee.flavor='burned'
coffee.cup = 'small'
coffee.price = 1.09
table_service.insert_entity('itemstable', coffee)
print('Created entry for Terible and Fake "parisian"...\n')
time.sleep(1)

coffee = Entity()
coffee.PartitionKey = 'coffeeshop'
coffee.RowKey = '006'
coffee.brand = 'Van houte'
coffee.flavor='columbian'
coffee.cup = 'small'
coffee.price = 1.99
table_service.insert_entity('itemstable', coffee)
print('Created entry for Van Houte...\n')
time.sleep(1)

coffee = Entity()
coffee.PartitionKey = 'coffeeshop'
coffee.RowKey = '007'
coffee.brand = 'Saint Denis'
coffee.flavor='Kenya AAA'
coffee.cup = 'small'
coffee.price = 2.39
table_service.insert_entity('itemstable', coffee)
print('Created entry for Yummy Kenya...\n')
time.sleep(1)

###
# Use the Azure Storage Storage SDK for Python to query for entities in our Table
###
print('With some data in our Azure Storage Table, we can query the data.\nLet\'s see what the car and coffeeshops looks like.')
raw_input('Press Enter to continue...')
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'car'", select='description,make,color,year,cost')
for item in items:
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'coffeeshop'", select='brand,flavor,cup,price')
for item in items:
    print('Brand:     ' + item.brand)
    print('flavor:    ' + item.flavor)
    print('cup size:  ' + item.cup)
    print('cup price: ' + str(item.price) + '\n')
